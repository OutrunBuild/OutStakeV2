...
stale_evidence_remediation_command="$(node ./script/process/read-process-config.js policy quality_gate.stale_evidence_remediation_command 2>/dev/null || printf '%s' 'npm run stale-evidence:loop')"
stale_evidence_exit_code="$(node ./script/process/read-process-config.js policy quality_gate.stale_evidence_exit_code 2>/dev/null || printf '%s' '2')"
...
for file in "${solidity_candidates[@]}"; do
    [ -z "$file" ] && continue
    if [[ "$file" =~ $src_sol_pattern ]] && [ -f "$file" ]; then
