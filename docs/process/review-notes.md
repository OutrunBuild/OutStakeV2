...
`docs/reviews/*.md` 默认是本地 review 草稿目录。命中 `src/**/*.sol`、`script/**/*.sol` 时，本地 `quality:gate:fast`、本地与 CI 的 `quality:gate` 都要求至少有一份有效 review note，作为安全、简化、Gas 与验证证据。

spec surface 不新增独立 review note；spec review evidence 是 spec surface 的 reviewer artifact。当前 gate 会同时消费路径命中和 brief 声明的 spec 产物，并对 `Task Brief`、writer Agent Report、spec review evidence 与 `verifier` 元数据链做 machine check。

前置说明：

- 当前仓库已经落盘 `script/process/check-review-note.sh`、`script/process/check-solidity-review-note.sh`、`script/process/check-spec-reviewer-evidence.sh`、`quality:quick`、`quality:gate:fast` 与 `quality:gate`。execution-plane 对 review-note 的消费范围仍限于 Solidity review note 约束；spec surface 则由 spec review evidence 和专用 evidence check 收口。
...
- spec surface 不纳入 review note 体系；相关证据由 `Task Brief`、writer Agent Report、spec review evidence 与 `verifier` 串联，freshness 与 required artifacts / commands 由 spec evidence check 独立拦截，不要求新增 review note spec 字段。
- 对 `prod-semantic` / `high-risk` 的 `src/**/*.sol`、`script/**/*.sol` 写面，本地 `quality:gate:fast` / `quality:gate`（含 `pre-commit`）会在进入 review-note / verifier 校验前自动再执行一次 `npm run codex:review`；`pre-push` / CI 只校验证据链，不自动执行。`non-semantic` / `test-semantic` 与流程面默认按需手动触发。若当前交互会话支持 `/review`，可视为同义入口，但落盘 evidence 仍以 wrapper / CLI 命令为准。
...
- 如果 writer 再次写入同一 spec scope，上一轮 spec review evidence 也会被视为 stale；它应在最新 writer Agent Report 之后重新生成，stale-evidence loop 会按当前 brief 声明的 spec scope 重新派发 writer / spec-reviewer / verifier。
...
- stale-evidence freshness 只针对当前 classifier 选中的 reviewer / verifier evidence；spec surface 的 `spec-reviewer Agent Report` 在契约上属于必选 reviewer evidence，stale 即 blocker。
- 发现 stale evidence 后，`quality:gate:fast` / `quality:gate` 会默认触发 `npm run stale-evidence:loop`；它会生成新的 follow-up brief，并给出 writer / reviewer / verifier 的 rerun order。spec surface 的 stale spec review evidence 也走同一自动 remediation。
...
- 命中 `src/**/*.sol`、`script/**/*.sol` 变更且准备运行本地 `quality:gate:fast`、本地或 CI `quality:gate` 时，必须先准备好一份可通过校验的 review note。
