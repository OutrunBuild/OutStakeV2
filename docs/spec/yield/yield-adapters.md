...
> `OutrunWstETHSYUpgradeable.sol::_previewDeposit` 的 NATIVE 路径只执行一次 `getSharesByPooledEth` 报价；`OutrunWstETHSYUpgradeable.sol::_deposit` 实际执行 `submit -> getPooledEthByShares -> wrap`，上游 floor 舍入可能使实际 SY 输出低于 preview。固定主网 fork 证据显示当前状态下偏差最多 1 wei；调用方将该报价用于 `SYBaseUpgradeable.sol::deposit` 的 `minSharesOut` 或组合路由最小值时，应预留协议舍入与状态变化余量。
