...
- `SYBaseUpgradeable.redeem` 采用 token-out-before-burn 的结算顺序：adapter `_redeem`（含外部调用）先把 tokenOut 交付给 receiver，随后 `_burn` 才烧掉 SY 份额；重入安全完全依赖 `redeem` 上的 `nonReentrant`（router 路径 `burnFromInternalBalance=true` 从 `address(this)` 烧、直调路径 `burnFromInternalBalance=false` 从 `msg.sender` 烧）。这条 CEI 逆序是有意设计，调整顺序或移除 `nonReentrant` 前必须保留该不变量。
...
> assetInfo 列：NATIVE sentinel 指 assetAddress = address(0)，表示 canonical asset 为原生 ETH/BNB（WeETH/SlisBNB/AsBNB）；L2 族的 canonical asset decimals 由部署期构造/storage 参数配置，position 在 init 缓存后不再重读，错误值会被静默缓存。

## Aave aToken 接口消费面

`OutrunAaveV3SYUpgradeable` 通过本地 `IAToken` 接口（`src/integrations/aave/interfaces/IAToken.sol`）消费以下 aToken 成员：

- `UNDERLYING_ASSET_ADDRESS()`：在 `initialize` 中绑定 adapter 的 underlying（`OutrunAaveV3SYUpgradeable.sol:42`）
- `scaledBalanceOf(user)`：在 underlying 存款路径中按流动性指数前后差值计量 scaled shares（`OutrunAaveV3SYUpgradeable.sol:73,76`）
