...
- adapter 自身做 raw answer 正性检查与 `maxStaleness` 新鲜度窗口校验（含 `updatedAt == 0` fail-closed）及可选构造期 L2 sequencer 校验，并在归一化后校验结果非零（`ZeroNormalizedRate`）；不提供 heartbeat、deviation bounds、fallback 或多源聚合保证
