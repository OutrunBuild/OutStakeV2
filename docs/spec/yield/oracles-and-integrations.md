...
- 构造期边界：`OutrunExchangeOracleAdapter.sol::constructor` 拒绝 `_maxStaleness == 0`（revert `InvalidStaleness`）；零值不代表关闭新鲜度检查——零窗口下任何 `updatedAt < block.timestamp` 的异步 feed 均判 stale，等同永久不可用，故 fail-fast 于构造期；`InvalidStaleness` 属构造期错误，不属 `IExchangeRateOracle.sol` 声明的 5 个具名运行期错误之列（与 `Panic(0x11)` 同一分隔）
