...
- Position 侧带宽护栏（PA-3 / G-1/G-2 消费端）：`OutrunStakingPositionUpgradeable.sol::_currentExchangeRate` 为全链路唯一速率读取点，除 `rate != 0` 外新增可选带宽 `minExchangeRate`/`maxExchangeRate`（0/0 表示禁用，兼容已部署代理），`setExchangeRateBounds` owner-only，`max != 0` 时 `rate ∉ [min,max]` 即 `ExchangeRateOutOfBounds`。与适配器层的 `maxStaleness`/`sequencer` 分层：适配器保新鲜度/可用性，position 保带宽；部署期 `script/lib/L2AssetValidation.sol:validateL2OracleAdapter` 对适配器 fail-fast，运行期 position 侧带宽为第二道本地护栏。`docs/deployment.md` 与 `docs/spec/position/state-machines.md` 为双端文档锚点。
