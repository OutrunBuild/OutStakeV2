...
- 单位约定：`OutrunRateLimiterUpgradeable.sol::RateLimit` 的 `limit` / `amountInFlight`、`OutrunRateLimiterUpgradeable.sol::RateLimitConfig` 的 `limit` 与 `OutrunRateLimiterUpgradeable.sol::_amountCanBeSent` 返回的 `amountCanBeSent` 均在 LD（local decimals）域，与 `OutrunOFTUpgradeable.sol::_debit` 传入 `OutrunRateLimiterUpgradeable.sol::_outflow` 的 `amountSentLD` 同单位；`OutrunOFTUpgradeable.sol::setOutboundRateLimit` 的 `limit` 形参与部署脚本 env `*_OUTBOUND_RATE_LIMIT` 均按 LD 解释。18-dec 部署（DCR = 1e12）下 1 token = 1e18 LD，配置限额用 LD 全值（如 1000000e18），切勿按 SD（shared decimals，6 位）习惯填写——`1e12`（= 1 个 SD 单位 = dust 阈值）会被当作 LD 使用，任何真实出站立即 `RateLimitExceeded`
...
- 实现 initializer lock：`OutrunOFTUpgradeable.sol::constructor` 调用 `_disableInitializers()`，故 implementation 本尊不可被直接 `initialize`（直接调用回退 `Initializable.InvalidInitialization`），只能经 `ERC1967Proxy` delegatecall 走 `OutrunUniversalAssetsUpgradeable.sol::initialize` 初始化；由 `test/upgradeable/OutrunUniversalAssetsUpgradeable.t.sol::testImplementationCannotBeInitializedDirectly` 锁定；`OutrunStakingPositionUpgradeable.sol::constructor` 与 `SYBaseUpgradeable.sol::constructor` 同样禁用 initializer
