...
- Summary: 新增 `OutrunAsBNBSY`、Aster 最小接口、mock/unit/fuzz/fork 测试，并在三轮 follow-up 中把 `exchangeRate()` 修正为 `asBNB -> slisBNB -> BNB` 闭环、把 constructor zero-check 前移到 `SYBase(...)` 参数求值之前、把 zero-share 与 queued mint 错误语义拆分为 `AsBnbMintZeroShares` / `AsBnbMintQueued`、最后补齐 gate 要求的 NatSpec。
