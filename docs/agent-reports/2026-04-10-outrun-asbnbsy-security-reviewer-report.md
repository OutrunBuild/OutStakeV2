...
- Summary: 最终 security review 未发现 confirmed finding。constructor 接线校验、`AsBnbMintZeroShares/AsBnbMintQueued` 分流、Aster queue `return 0 -> revert`、`redeem(asBNB)` 限定 token universe 和 unit/fuzz/fork 失败路径覆盖都足以支撑当前变更集。
...
- Evidence:
  ...
  - `sharesOut == 0` 且非 queue 时现在改为 `AsBnbMintZeroShares`，避免把 dust/zero-share 与真实 queue 混成同一错误语义
